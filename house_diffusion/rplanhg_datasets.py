...
class RPlanhgDataset(Dataset):
    def __init__(self, set_name, analog_bit, target_set, non_manhattan=False):
        ...
        os.makedirs("processed_rplan", exist_ok=True)
        super().__init__()
        ...
        if self.set_name == "eval":
            # Automatically generate training distribution if missing
            train_cndist_path = f"processed_rplan/rplan_train_{target_set}_cndist.npz"
            if not os.path.exists(train_cndist_path):
                # Process raw training data to create required files
                RPlanhgDataset("train", analog_bit, target_set)
            cnumber_dist = np.load(train_cndist_path, allow_pickle=True)[
                "cnumber_dist"
            ].item()
        if os.path.exists(f"processed_rplan/rplan_{set_name}_{target_set}.npz"):
            data = np.load(
                f"processed_rplan/rplan_{set_name}_{target_set}.npz", allow_pickle=True
            )
            ...
            if self.set_name == "eval":
                data = np.load(
                    f"processed_rplan/rplan_{set_name}_{target_set}_syn.npz",
                    allow_pickle=True,
                )
            ...
            for graph in tqdm(self.subgraphs):
                ...
                for room_mask, room_type in zip(rooms_mks, graph_nodes):
                    ...
                    room_mask = cv.resize(
                        room_mask, (256, 256), interpolation=cv.INTER_AREA
                    )
            ...
            if self.non_manhattan:
                for h, graph in tqdm(
                    zip(self.org_houses, self.org_graphs), desc="processing dataset"
                ):
            ...
            for h, graph in tqdm(
                ...
                for i, room in enumerate(h):
                    ...
                    room[0] = (
                        np.reshape(room[0], [len(room[0]), 2]) / 256.0 - 0.5
                    )  # [[x0,y0],[x1,y1],...,[x15,y15]] and map to 0-1 - > -0.5, 0.5
                    ...
                    rtype = np.repeat(
                        np.array([get_one_hot(room[1], 25)]), num_room_corners, 0
                    )
                    room_index = np.repeat(
                        np.array([get_one_hot(len(house) + 1, 32)]), num_room_corners, 0
                    )
                    corner_index = np.array(
                        [get_one_hot(x, 32) for x in range(num_room_corners)]
                    )
                    ...
                    connections = np.array(
                        [
                            [i, (i + 1) % num_room_corners]
                            for i in range(num_room_corners)
                        ]
                    )
                ...
                for i in range(len(corner_bounds)):
                    for j in range(len(corner_bounds)):
                        if i == j:
                            self_mask[
                                corner_bounds[i][0] : corner_bounds[i][1],
                                corner_bounds[j][0] : corner_bounds[j][1],
                            ] = 0
                        elif any(np.equal([i, 1, j], graph).all(1)) or any(
                            np.equal([j, 1, i], graph).all(1)
                        ):
                            door_mask[
                                corner_bounds[i][0] : corner_bounds[i][1],
                                corner_bounds[j][0] : corner_bounds[j][1],
            ...
            if self.set_name == "train":
                np.savez_compressed(
                    f"processed_rplan/rplan_{set_name}_{target_set}_cndist",
            ...
            if set_name == "eval":
                ...
                for h, graph in tqdm(
                    ...
                    num_room_corners_total = [
                        cnumber_dist[room[1]][
                            random.randint(0, len(cnumber_dist[room[1]]) - 1)
                        ]
                        for room in h
                    ]
                    ...
                    for i, room in enumerate(h):
                        ...
                        room_index = np.repeat(
                            np.array([get_one_hot(len(house) + 1, 32)]),
                            num_room_corners,
                            0,
                        )
                        corner_index = np.array(
                        ...
                        connections += num_points
                        corner_bounds.append(
                            [num_points, num_points + num_room_corners]
                        )
                        num_points += num_room_corners
                        room = np.concatenate(
                            (
                                np.zeros([num_room_corners, 2]),
                                rtype,
                                corner_index,
                                room_index,
                                padding_mask,
                    ...
                    for i in range(len(corner_bounds)):
                        ...
                        if not is_connected:
                            door_mask[
                                corner_bounds[i][0] : corner_bounds[i][1],
                                corner_bounds[living_room_index][0] : corner_bounds[
                                    living_room_index
                                ][1],
                            ] = 0
                ...
                np.savez_compressed(
                    f"processed_rplan/rplan_{set_name}_{target_set}_syn",
                    graphs=self.syn_graphs,
                    houses=self.syn_houses,
                    door_masks=self.syn_door_masks,
                    self_masks=self.syn_self_masks,
    ...
    def __getitem__(self, idx):
        ...
        graph = np.concatenate(
            (self.graphs[idx], np.zeros([200 - len(self.graphs[idx]), 3])), 0
        )
        ...
        cond = {
            ...
            "corner_indices": self.houses[idx][
                :, self.num_coords + 25 : self.num_coords + 57
            ],
            "room_indices": self.houses[idx][
                :, self.num_coords + 57 : self.num_coords + 89
            ],
            "src_key_padding_mask": 1 - self.houses[idx][:, self.num_coords + 89],
            "connections": self.houses[idx][
                :, self.num_coords + 90 : self.num_coords + 92
            ],
            "graph": graph,
        }
        if self.set_name == "eval":
            syn_graph = np.concatenate(
                (self.syn_graphs[idx], np.zeros([200 - len(self.syn_graphs[idx]), 3])),
                0,
            )
            ...
            cond.update(
                {
                    ...
                    "syn_room_types": self.syn_houses[idx][
                        :, self.num_coords : self.num_coords + 25
                    ],
                    "syn_corner_indices": self.syn_houses[idx][
                        :, self.num_coords + 25 : self.num_coords + 57
                    ],
                    "syn_room_indices": self.syn_houses[idx][
                        :, self.num_coords + 57 : self.num_coords + 89
                    ],
                    "syn_src_key_padding_mask": 1
                    - self.syn_houses[idx][:, self.num_coords + 89],
                    "syn_connections": self.syn_houses[idx][
                        :, self.num_coords + 90 : self.num_coords + 92
                    ],
                    "syn_graph": syn_graph,
                }
